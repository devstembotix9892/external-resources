...
void drawCustom(const uint8_t symbol[35], uint32_t color) {
  matrix.clear();
...
`;
    ...
    // ================================
    // CUSTOM PATTERN (FIXED)
    // ================================
    Blockly.Arduino.ledmatrix_draw_custom = function (block) {

        const matrixVal = block.getFieldValue('MATRIX');

        const varName = Blockly.Arduino.variableDB_.getDistinctName(
            'customSymbol',
            Blockly.Variables.NAME_TYPE
        );

        let code = `uint8_t ${varName}[35] = {`;
        ...
        code += '};\n';
        code += `drawCustom(${varName}, matrix.Color(0,255,0));\n`;
